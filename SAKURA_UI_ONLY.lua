jhadstring(game:HttpGet("https://raw.githubusercontent.com/ffgringoxp123-code/Snns/refs/heads/main/Bbb"))()",
        SliderValue = "Value",
...
local Config = setmetatable({
    save = function(self: any, file_name: any, config: any)
        local success_save, result = pcall(function()
            ...
            writefile('SakuraUI/'..file_name..'.json', flags)
        end)
    ...
    load = function(self: any, file_name: any, config: any)
        local success_load, result = pcall(function()
            if not isfile('SakuraUI/'..file_name..'.json') then
                self:save(file_name, config)
        
                return
            end
        
            local flags = readfile('SakuraUI/'..file_name..'.json')
...
function Library:create_ui()
    local old_SakuraUI = CoreGui:FindFirstChild('SakuraUI')

    if old_SakuraUI then
        Debris:AddItem(old_SakuraUI, 0)
    end

    local SakuraUI = Instance.new('ScreenGui')
    SakuraUI.ResetOnSpawn = false
    SakuraUI.Name = 'SakuraUI'
    SakuraUI.ZIndexBehavior = Enum.ZIndexBehavior.Sibling
    SakuraUI.Parent = CoreGui
...
Background.Image="rbxassetid://124990780858879"
...
Background.ZIndex=0
    ...
    ClientName.TextColor3 = Color3.fromRGB(255, 183, 197)
...
Icon.Image = "rbxassetid://100375298524189"
    ...
    function self:UIVisiblity()
        SakuraUI.Enabled = not SakuraUI.Enabled;
    end;
    ...
    function self:load()
        ...
        for _, object in SakuraUI:GetDescendants() do
